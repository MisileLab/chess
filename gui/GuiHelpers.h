...
/*
Creates a transparent texture from the specified bmp. The user has to specify
the color that he want to make transparent.
@param rend the renderer
@param imagePath the image
@param r, g, b the color
@return a texture, or NULL if failed
*/
SDL_Texture *guiTransparentTextureFromBMP(SDL_Renderer *rend, char *imagePath,
                                          Uint8 r, Uint8 g, Uint8 b);

void guiPushUserEvent(GuiUserEventCode code, void *data1, void *data2);

/*
Displays a simple message box. If the call to SDL fails, prints the msg in the
console.
