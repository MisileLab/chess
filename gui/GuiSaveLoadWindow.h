...
/*
This module represents the save-load window. It dynamically loads the correct
page. The number of saving slots is changeable using the first definition here.
When changing the number of slots, relevant images have to be created in the img
directory with the following names: button_slotX - switching X with the relevant
number button_slotX_empty - represents an empty slot (e.g not saved)
...
typedef struct save_load_window_t {
  SDL_Window *window;
  SDL_Renderer *rend;
  GuiWidget *widgets[GUI_NUMBER_OF_SAVE_SLOTS +
                     3]; // back, next slots, prev slots, slots
...
*/
GuiWindow *guiSaveLoadWindowCreate(SDL_Window *window, SDL_Renderer *rend,
