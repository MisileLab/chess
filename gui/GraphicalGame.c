...
GraphicalGame *graphicalGameCreate() {
  ...
  gg->window =
      SDL_CreateWindow("Chess", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                       GUI_WINDOW_WIDTH, GUI_WINDOW_HEIGHT, SDL_WINDOW_SHOWN);
...
void graphicalGameDestroy(GraphicalGame *gg) {
  ...
  if (gg->rend != NULL)
    SDL_DestroyRenderer(gg->rend);
  if (gg->window != NULL)
    SDL_DestroyWindow(gg->window);

  if (gg->curWindow != NULL)
    gg->curWindow->destroy(gg->curWindow);

  // we need the next line in case user click X on save window
  if (gg->gameWindowBeforeSaveLoad != NULL)
    gg->gameWindowBeforeSaveLoad->destroy(gg->gameWindowBeforeSaveLoad);
...
static void guiSwitchWindow(GraphicalGame *gg, GuiWindowEnum nextWindow) {
  ...
  case GUI_WINDOW_SAVE:
    gg->curWindow = guiSaveLoadWindowCreate(
        gg->window, gg->rend, true,
  ...
  case GUI_WINDOW_GAME:
    ...
    if (gg->gameWindowBeforeSaveLoad) {
      ...
      gg->curWindow = guiGameWindowCreate(gg->window, gg->rend,
                                          gameHandlerNewGame(gg->settings));
    }
  ...
  case GUI_WINDOW_GAME_FROM_LOAD:
    ...
    gg->curWindow =
        guiGameWindowCreate(gg->window, gg->rend, gg->loadedGameHandler);
    gg->lastWindow = GUI_WINDOW_WELCOME;
  default:
    break;
  }
...
/*
This is a wrapper for switching to save or load from the game window, because in
this case we shouldn't dispose the current window.
*/
static void guiSwitchToSaveLoadFromGameWindow(GraphicalGame *gg,
                                              GuiWindowEnum nextWindow) {
  if (!gg ||
      ((nextWindow != GUI_WINDOW_LOAD) && (nextWindow != GUI_WINDOW_SAVE)))
    return;
...
static void guiHandleUserEventGameModeChosen(GraphicalGame *gg,
                                             GhGameMode gameMode) {
...
static void
guiHandleUserEventDifficultyChosen(GraphicalGame *gg,
                                   GhGameDifficultyLevel difficulty) {
  gg->settings.difficultyLevel = difficulty;
  guiSwitchWindow(gg, GUI_WINDOW_USER_COLOR);
}

static void guiHandleUserEventColorChosen(GraphicalGame *gg,
                                          GhUserColor color) {
...
static void guiHandleUserEventLoadSlotChosen(GraphicalGame *gg,
                                             GameHandler *gh) {
  if (gh == NULL) {
...
/*
userWantsToQuit - if true, self explanatory. if false, user wants to get to the
main menu Returns true if the caller has to take control (cancel or no)
*/
static bool guiPromtForUnsavedGame(GraphicalGame *gg, bool userWantsToQuit) {
  ...
  SDL_MessageBoxButtonData buttons[] = {
      ...
      {SDL_MESSAGEBOX_BUTTON_ESCAPEKEY_DEFAULT, MSGBOX_CANCEL_BUTTON_ID,
       "cancel"},
  };
  ...
  case MSGBOX_YES_BUTTON_ID:
    if (userWantsToQuit)
      gameWindow->exitAfterSave = true;
    else
      gameWindow->menuAfterSave = true;
...
int graphicalGameRun() {
  ...
  while (!(gg->errorOrStop)) {
    ...
    // user event
      ...
      case GUI_USEREVENT_DIFFICULTY_CHOSEN:
        guiHandleUserEventDifficultyChosen(gg,
                                           (GhGameDifficultyLevel)e.user.data1);
        break;
      case GUI_USEREVENT_COLOR_CHOSEN:
        guiHandleUserEventColorChosen(gg, (GhUserColor)e.user.data1);
        break;
      case GUI_USEREVENT_MSGBOX:
        guiShowMessageBox((const char *)e.user.data1,
                          (const char *)e.user.data2);
        break;
      ...
      case GUI_USEREVENT_MENU_FROM_GAME_WINDOW:
        if (guiPromtForUnsavedGame(gg, false))
          guiSwitchWindow(gg, GUI_WINDOW_WELCOME);
        break;
      // the following are cases where the current window (i.e gameBoard) has to
      // take control
  ...
  // SDL_Quit fires automatically atexit. see start of function (taken from SDL
  // docs)
  graphicalGameDestroy(gg);
  return 0;
}
